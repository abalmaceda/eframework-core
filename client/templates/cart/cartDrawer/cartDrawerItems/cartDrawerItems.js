...
Template.cartDrawerItems.helpers({
	media: function () {
		...
		let defaultImage = EFrameworkCore.Collections.Media.findOne({ "metadata.variantId": this.variants._id });
		...
		if (defaultImage) {
			...
			_.any(product.variants, function (variant) {
				defaultImage = EFrameworkCore.Collections.Media.findOne({"metadata.variantId": variant._id });
				return !!defaultImage;
			});
		}
		return defaultImage;
